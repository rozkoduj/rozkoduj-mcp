...
class TestAnalyze:
    ...
    @pytest.mark.anyio
    @patch("rozkoduj_mcp.tools.analyze.scanner")
    async def test_returns_analysis(self, mock_scanner: MagicMock) -> None:
        mock_scanner.analyze = AsyncMock(return_value=mock_analysis())

        result = await analyze("BTCUSDT")

        mock_scanner.analyze.assert_called_once_with("BTCUSDT", "1d")
        assert result["summary"]["recommendation"] == "BUY"

    @pytest.mark.anyio
    @patch("rozkoduj_mcp.tools.analyze.scanner")
    async def test_custom_interval(self, mock_scanner: MagicMock) -> None:
        mock_scanner.analyze = AsyncMock(return_value=mock_analysis())

        await analyze("AAPL", interval="4h")

        mock_scanner.analyze.assert_called_once_with("AAPL", "4h")

    @pytest.mark.anyio
    @patch("rozkoduj_mcp.tools.analyze.scanner")
    async def test_result_has_indicators(self, mock_scanner: MagicMock) -> None:
        mock_scanner.analyze = AsyncMock(return_value=mock_analysis())
