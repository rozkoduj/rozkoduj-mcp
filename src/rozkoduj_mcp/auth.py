"""JWT identity extraction bound to request-scoped ContextVars."""
        ...
        for attempt in range(2):
            ...
            try:
                return jwt.decode(
                    ...
                    leeway=30,
                    # Reject a token that omits exp: without this PyJWT only
                    # validates exp when present, so a missing claim would be
                    # accepted as a non-expiring bearer (RFC 8725).
                    options={"require": ["exp"]},
                )
