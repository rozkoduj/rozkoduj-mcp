...
class TestGetAnalysis:
    ...
    async def test_delegates_to_api(self, mock_scanner: MagicMock) -> None:
        ...
        result = await get_analysis("BTCUSDT", "1d")

        mock_scanner.analyze.assert_called_once_with(symbol="BTCUSDT", interval="1d")
        assert result["summary"]["recommendation"] == "BUY"

    @pytest.mark.anyio
    @patch("rozkoduj_mcp.services.ta.scanner")
    async def test_passes_interval(self, mock_scanner: MagicMock) -> None:
